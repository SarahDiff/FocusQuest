...
import { DISCIPLINE_META, formatDuration } from "@/lib/fq-data";
  ...
  return (
    <div className="grid grid-cols-3 gap-3 mb-6">
      {stats.map(s => (
        ...
        >
          <div className="font-display font-semibold mb-1" style={{ fontSize: 15, color: 'var(--fq-xp-bright)', textShadow: '0 0 12px rgba(212,168,75,0.3)' }}>
            {s.value}
          </div>
          <div className="font-display uppercase" style={{ fontSize: 7, letterSpacing: '0.14em', color: 'var(--fq-text-muted)' }}>
            {s.label}
          </div>
        </div>
      ))}
        ...
        {Array.from({ length: daysInMonth }, (_, i) => {
          ...
          return (
            <div
              ...
              style={{
                background: mins > 0 ? `rgba(94,196,192,${intensity * 0.35})` : 'rgba(255,255,255,0.03)',
                border: isToday ? '1.5px solid var(--fq-teal)' : mins > 0 ? '1px solid rgba(94,196,192,0.2)' : '1px solid transparent',
                boxShadow: isToday ? '0 0 6px rgba(94,196,192,0.3)' : 'none',
              }}
            >
              <span className="font-display" style={{ fontSize: 8, color: isToday ? 'var(--fq-teal)' : mins > 0 ? 'var(--fq-text-body)' : 'var(--fq-text-muted)', letterSpacing: '0.04em' }}>
                {day}
              </span>
            </div>
          );
...
export default function History() {
  ...
  return (
    <div className="px-5 pt-10 pb-4">
      <div className="mb-6">
        <p className="font-display uppercase mb-1" style={{ fontSize: 9, letterSpacing: '0.26em', color: 'var(--fq-teal)', opacity: 0.8 }}>
          Your Journey
        </p>
      ...
      <div className="mb-4">
        ...
        {sessions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="font-display font-medium mb-2" style={{ fontSize: 16, color: 'var(--fq-text-body)', letterSpacing: '0.04em' }}>No Sessions Yet</h3>
            <p className="font-serif italic" style={{ fontSize: 14, color: 'var(--fq-text-muted)' }}>Begin a focus session to write your legend.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {sessions.map((sess, i) => {
              const meta = DISCIPLINE_META[sess.skillId as keyof typeof DISCIPLINE_META];
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderLeft: `2.5px solid ${i === 0 ? 'var(--fq-xp)' : i < 3 ? 'rgba(212,168,75,0.4)' : 'rgba(212,168,75,0.15)'}`,
                ...
                >
                  <div
                    ...
                    style={{ width: 38, height: 38, background: 'rgba(94,196,192,0.06)', border: '1px solid var(--fq-border)' }}
                  >
                    <span className="font-display" style={{ fontSize: 16, color: 'var(--fq-teal)' }}>
                      {meta?.glyph ?? '◎'}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="font-display font-medium truncate" style={{ fontSize: 12, color: 'var(--fq-text-primary)', letterSpacing: '0.04em' }}>
                        {meta?.label ?? sess.skillName}
                      </span>
                    </div>
                  ...
                  <span className="font-display font-bold flex-shrink-0" style={{ fontSize: 14, color: 'var(--fq-xp-bright)', textShadow: '0 0 10px rgba(212,168,75,0.4)' }}>
                    +{sess.xpEarned}
                  </span>
                </div>
              );
